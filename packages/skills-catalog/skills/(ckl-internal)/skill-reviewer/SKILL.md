...
1. Run `scripts/pr_touched_skills.sh <PR-ref>` to list skill roots changed in the PR. The script is layout-agnostic — it walks the PR branch's git tree for every `SKILL.md` and maps each changed file to its nearest ancestor skill root. Accepts a PR number, `#N`, or a full GitHub URL.
   - **Cross-repo note:** if the URL points at a repo other than your current local clone (e.g., you're in `agent-skills` but the URL is `CheesecakeLabs/ckl-ai-skills/pull/27`), the script auto-detects via the URL form (`-R OWNER/REPO`). It then switches to **PR-relative path output** (no local prefix) and prints `CROSS_REPO_NO_FILES_ON_DISK` to stderr. Do NOT pipe those paths into `run_validate.sh` or `security_sweep.sh` — they don't exist on disk. Two fixes: either `cd` into a local clone of the matching repo and re-run (paths become absolute again), or pass `--checkout` to have the script do `gh pr checkout` for you (refuses on a dirty tree unless you also pass `--force`). `--checkout` brings the PR into the current clone and restores absolute-path output.
